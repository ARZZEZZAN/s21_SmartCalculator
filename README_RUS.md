...
А моральный дух был почти на исходе! Вдруг из-за двери донеслась фраза "он кажется в своем 
кабинете", и стремительные шаги толпы понеслись в сторону Криса.  Страх овладел им на момент, но он 
...
Пример выражения, написанного в обратной польской (безскобочной) нотации:
...
- Сборка программы должна быть настроена с помощью Makefile со стандартным набором целей для GNU-программ: all, install, uninstall,clean, distclean, mostyclean, realclean, TAGS, info, dvi, dist. При этом установка должна вестись в каталог /opt/smartcalc/v1.0  
- Программа должна быть разработана в соответствии с принципами структруного программирования
- Должно быть обеспечено покрытие unit-тестами модулей, связанных с вычислением выражений
